...
export const fetchSitreps = async():Promise<Sitrep_Interface[] | Error_Interface>  => {
    try{
        const response = await Sitrep.find({}, '_id sitrepImage description account viewers').populate("account viewers", "_id profilePicture userName").sort({createdAt: -1}).lean<Sitrep_Interface[]>()
        return response
    }catch(err: unknown){
